...
function compile_defs(defs, main, opts) {
  ...
  if (isio) {
    ...
    code += "          case 'get_file': try { run_io(lib, p.then(lib.fs.readFileSync(p.param,'utf8'))).then(res).catch(err); } catch (e) { if (e.message.indexOf('NOENT') !== -1) { run_io(lib, p.then('')).then(res).catch(err); } else { err(e); } }; break;\n";
