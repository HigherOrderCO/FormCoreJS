...
function compile_defs(defs, main, opts) {
  ...
  if (isio) {
    ...
    code += "        switch (p.query) {\n";
    code += "          case 'print': console.log(p.param); run_io(lib, p.then('')).then(res).catch(err); break;\n";
    ...
    code += "          case 'request': try { request(lib,p.param).then(x => run_io(lib, p.then(x))).then(res).catch(err); } catch (e) { err(e); }; break;\n";
